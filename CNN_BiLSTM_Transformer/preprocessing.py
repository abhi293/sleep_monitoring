...
DELTA_COLS = [f"{col}_delta" for col in DELTA_SOURCES]
...
def user_split(
    ...
    if len(users) < 3:
        ...
        val_df, test_df = _row_split(tmp_df, val_ratio / max(val_ratio + test_ratio, 1e-8), seed + 1)
...
def _window_group(group: pd.DataFrame, window_size: int, stride: int, feature_cols: List[str]):
    ...
    for start in range(0, len(group) - window_size + 1, stride):
        xs.append(feats[start:start + window_size])
...
def create_windows(
    ...
    if n_jobs == 1:
        results = [_window_group(g, window_size, stride, feature_cols) for g in groups]
    else:
        results = Parallel(n_jobs=n_jobs, backend="loky")(
            delayed(_window_group)(g, window_size, stride, feature_cols) for g in groups
        )
...
def get_class_weights(y: np.ndarray, boost_minority: float = 1.15) -> Dict[int, float]:
    classes = np.unique(y)
